...
void MapUI::PrintMenu() {
    ...
    case 3:
  {
    ...
    menu = "*************************Results******************************\n";
    std::cout << menu;
    std::vector<std::string> vecCategories = map.GetAllCategories();
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    for(auto iter = vecCategories.begin(); iter!=vecCategories.end(); iter++){
  ...
  case 4:
  {
    ...
    menu = "*************************Results******************************\n";
    std::cout << menu;
    std::vector<std::pair<double,double>> vecLocations = map.GetAllLocationsFromCategory(input);
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    ...
    if(vecLocations.size() == 0){
      ...
      for(auto iter = vecLocations.begin(); iter!=vecLocations.end(); iter++){
        std::cout<<"( "<<iter->first<<" , "<<iter->second<<" )"<<std::endl;
      }
      PlotPoints(vecLocations);
    }
    menu = "**************************************************************\n";
    std::cout << menu;
    std::cout << "Time taken by function: " << duration.count()/1000 << " ms" << std::endl << std::endl;
  ...
  case 5:
  {
    ...
    menu = "*************************Results******************************\n";
    std::cout << menu;
    std::vector<std::pair<double,double>> vecLocations = map.GetLocationRegex(input);
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    ...
    if(vecLocations.size() == 0){
      ...
      if(vecLocations[0].first == -1 && vecLocations[0].second == -1){
...
/**
 * PlotPoints: Given a vector of location coordinates draws the points on the map (no path).
 * 
 * @param  {std::vector<std::pair<double, double>>} locations : points
 */
void MapUI::PlotPoints(std::vector<std::pair<double, double>> &locations){
  std::string image_path = cv::samples::findFile("src/lib/map.png");
  cv::Mat img = cv::imread(image_path, cv::IMREAD_COLOR);
  // cv::resize(img, img, cv::Size(img.cols, img.rows));
  for (auto x : locations) {
    auto result = GetPlotLocation(x.first, x.second);
    cv::circle(img, cv::Point(result.first, result.second), DOT_SIZE,
               cv::Scalar(0, 0, 255), cv::FILLED);
  }
  cv::startWindowThread();
