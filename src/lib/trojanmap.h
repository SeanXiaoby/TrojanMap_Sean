...
class TrojanMap {
 public:
  ...
  std::vector<std::pair<double, double>> GetLocationRegex(std::string location);
  ...
  std::string StringToLower(std::string strInput);

  // Check if the location name exist in the data nodes
  bool FindLocationName(std::string location);

  //DFS_helper for topological sort in item8 DeliveryTrojan
  void DFS_helper(std::string &root, std::unordered_map<std::string, std::vector<std::string>> &hash_map, std::map< 
                std::string, int> &marks, std::vector<std::string> &topo_list);

  // Brute_force permutation helper for Item 9
  void BruteForce_helper(std::vector<std::string> &location_ids, 
                            std::pair<double, std::vector<std::vector<std::string> > > &results, 
                            std::pair<double, std::vector<std::string> > curPath);
  
  // Backtracking w/ early stopping helper for Item 9
  ...
  std::vector<std::string> ReorganizePath(std::string first_item, std::vector<std::string> input_path);


  static bool mycmp(std::pair<std::string, double> a1, std::pair<std::string, double> a2);
};
